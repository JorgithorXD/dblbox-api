...
router.get('/all', async (req, res) => {
    try {
        ...
        const dataUnitBasicEs = await getData.getAll('unit_basic_fr')
        const dataUnitAbilities = await getData.getAll('unit_abilities_fr')
        const dataUnitArts = await getData.getAll('unit_arts_fr')
        const dataUnitZenkai = await getData.getAll('unit_zenkai_fr')
        const dataUnitZenkaiArts = await getData.getAll('unit_arts_zenkai_fr')
        const dataUnitSrc = await getData.getAll('unit_src')

        const unit = dataUnitBasic.map((basic) => {
            const name = dataUnitBasicEs.find(name => name.id === basic.id)
